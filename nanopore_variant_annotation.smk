...
final_merged_vcf = output_directory+'/clair3_multisample/merged_multisample.vcf.gz'
...
module snp_eff_annotation:
	snakefile:
		github("simkin-bioinformatics/snp_eff_annotation", path = "/workflow/Snakefile", commit = "978e100c3434661dc47f2620cd591d581be70d43")
...
use rule run_snp_eff from snp_eff_annotation as snpeff_run_snp_eff with:
	input:
		snp_eff_config = output_directory+'/snpEff_processing/snpEff.config',
		snp_eff_database = output_directory+'/snpEff_processing/data/'+config['genome_database_name']+'/sequence.bin',
...
rule all:
	input:
		...
		copied_snakefile=output_directory+'/snakemake_params/nanopore_variant_annotation.smk',
		copied_yaml=output_directory+'/snakemake_params/nanopore_variant_annotation.yaml',
		merged_vcf=output_directory+'/clair3_multisample/merged_multisample.vcf.gz',
...
rule copy_files:
	...
	output:
		copied_snakefile=output_directory+'/snakemake_params/nanopore_variant_annotation.smk',
		copied_yaml=output_directory+'/snakemake_params/nanopore_variant_annotation.yaml',
		copied_targets = output_directory+'/config/targets.tsv',
		copied_config = output_directory+'/config/config.yaml'
	shell:
		'''
		cp {input.original_snakefile} {output.copied_snakefile}
		cp {input.original_yaml} {output.copied_yaml}
		cp {input.original_yaml} {output.copied_config}
...
rule run_minimap:
	...
	output:
		aligned_sam=output_directory+'/sam_files/{sample}_output.sam'
	shell:
...
rule sam_to_bam:
	input:
		aligned_sam=output_directory+'/sam_files/{sample}_output.sam'
	output:
		unsorted_bam=output_directory+'/unsorted_bam_files/{sample}_output.bam'
	shell:
...
rule sort_bam:
	input:
		unsorted_bam=output_directory+'/unsorted_bam_files/{sample}_output.bam'
	output:
		sorted_bam=output_directory+'/sorted_bam_files/{sample}_output_sorted.bam'
	shell:
...
rule index_bam:
	input:
		sorted_bam=output_directory+'/sorted_bam_files/{sample}_output_sorted.bam'
	output:
		indexed_bam=output_directory+'/sorted_bam_files/{sample}_output_sorted.bam.bai'
	shell:
...
rule run_clair3:
	...
	params:
		...
		clair3_folder=output_directory+'/clair3_samples/{sample}'
	output:
		clair3_vcf=output_directory+'/clair3_samples/{sample}/merge_output.vcf.gz'
	shell:
		'''
		run_clair3.sh \
...
rule index_vcf:
	input:
		clair3_vcf=output_directory+'/clair3_samples/{sample}/merge_output.vcf.gz'
	output:
		vcf_index=output_directory+'/clair3_samples/{sample}/merge_output.vcf.gz.csi'
	shell:
		'bcftools index {input.clair3_vcf}'

rule merge_vcfs:
	input:
		clair3_vcfs=expand(output_directory+'/clair3_samples/{sample}/merge_output.vcf.gz', sample=all_samples),
		vcf_indices=expand(output_directory+'/clair3_samples/{sample}/merge_output.vcf.gz.csi', sample=all_samples)
	output:
		merged_vcf=output_directory+'/clair3_multisample/merged_multisample.vcf.gz'
	shell:
		'''
...
# 		temp_config=output_directory+'/snp_eff_folder/snpEff.config'
# 	script:
...
# 		database_name=config['genome_database_name']
# 	output:
# 		snp_eff_stats=output_directory+'/snp_Eff_output/snpEff_summary.html',
# 		snp_eff_vcf=output_directory+'/snp_Eff_output/annotated_variants.vcf'
# 	shell:
# 		'''
...
# 	output:
# 		coverage_AA=output_directory+'/AA_tables/coverage_AA_table.csv',
# 		reference_AA=output_directory+'/AA_tables/reference_AA_table.csv',
# 		alternate_AA=output_directory+'/AA_tables/alternate_AA_table.csv',
# 	script:
